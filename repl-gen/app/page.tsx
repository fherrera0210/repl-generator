...
`
  ...
  const copyToClipboard = async () => {
    if (!generatedScript) {
      toast({
        title: "No REPL",
        description: "Please generate a REPL first",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Copied",
        description: "REPL copied to clipboard",
      })
    } catch (err) {
      toast({
        title: "Error",
    ...
    <div className={`min-h-screen ${theme === "dark" ? "dark" : ""}`}>
      <div className="bg-background text-foreground">
        ...
        <div className="flex justify-between items-center p-4 border-b border-border">
          <h1 className="text-2xl font-bold">Bulk Import REPL Gen</h1>
          <button
            onClick={toggleTheme}
          ...
          {generatedScript && (
            <Card className="p-6 border border-border mb-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-foreground">Generated REPL</h2>
                <button
                  onClick={() => setIsScriptExpanded(!isScriptExpanded)}
                  className="p-1 hover:bg-accent rounded transition-colors"
                  aria-label="Toggle repl section"
                >
